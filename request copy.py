...
def get_frame(q:None, time_:str, geo:str) -> pd.DataFrame:
    ...
    try:
        # print('\n',session.get("https://icanhazip.com/").text)
        jar = session.get("https://trends.google.com/").cookies
    except:
        print('error in session.get/n/n')
        time.sleep(20)
        jar = session.get("https://trends.google.com/").cookies
    # print('printing jar : ',jar)
    ...
    for idx ,interval in enumerate(tqdm(intervals)):
        ...
        while not flag:
            try:
                ...
                print('error in urllib.error.HTTPError \n',str(e))
                time.sleep(20)
                _renew_connection()
                # df = get_frame(q, interval, geo) 
            except requests.exceptions.SSLError as e :
                print('error in requests.exceptions.SSLError \n',str(e))
                time.sleep(20)
                _renew_connection()
                # df = get_frame(q, interval, geo)
                ...
                print('getting blocked from the server... handling it.. \n',str(e))
                time.sleep(20)
                _renew_connection()
                # df = get_frame(q, interval, geo)

        #print(len(df))
        time.sleep(random.gammavariate(.99,1.99)) # I should try to make a new condition > 3 seconds
        if len(df) == 0:
            continue
        frames.append(df)
    return frames
